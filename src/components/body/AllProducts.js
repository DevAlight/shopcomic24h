...
import { fetchData, changePagination, setModalLogin, addProductToCartHandler } from '../../actions/users.action';
...
const AllProducts = () => {
    ...
    const { user, dataFull, data, currentPage, quantity, pending } = useSelector((reduxUser) => reduxUser.userReducer);
    const saveData = JSON.parse(localStorage.getItem("item", "[]")) || [];
    const limit = 6;
    const noPage = Math.ceil(dataFull / limit);
    ...
    useEffect(() => {
        dispatch(fetchData(limit, currentPage));
    }, [currentPage])
    return (
        <Grid container>
            {pending ?
                ...
                <Grid item md={12} sm={12}>
                    <Box sx={{ backgroundColor: 'white', paddingBottom: '2px' }}>
                        ...
                        <Grid container mt={2} pb={2}>
                            <Stack >
                                <Pagination count={noPage} shape="rounded" defaultPage={currentPage} onChange={handleChange} />
                            </Stack>
                        </Grid>
                    </Box>
                </Grid>
