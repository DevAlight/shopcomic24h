...
const CartPage = () => {
    ...
    const onOrderClick = () => {
        ...
        const orderDetails = cartItems.map(({ Product, quantity }) => {
            ...
            return { product, name, imageUrl, promotionPrice, quantity }
        });
        let fixData = {
        ...
        dispatch(gettingOrder(userVerify._id, dataOrder))
    }
                                ...
                                {cartBag.map((value, index) => {
                                    return (
                                        <Box sx={styleBox} key={index}>
                                            <Grid container>
                                                <Grid item md={2} sm={2}>
                                                    <img className="banner-img" src={value.Product.imageUrl} alt='anhSP'/>
                                                </Grid>
